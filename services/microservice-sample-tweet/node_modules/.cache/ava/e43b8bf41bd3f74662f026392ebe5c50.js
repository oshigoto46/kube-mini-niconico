...
test.serial('get tweet not found', async t => {
  const res = await supertest(app).get(`/tweets/${new mongoose.Types.ObjectId()}`);
  t.is(res.status, 404);
...
test.serial('create tweet', async t => {
  const content = 'xxx';
  const res = await supertest(app).post('/tweets').send({
...
test.serial('delete tweet not found', async t => {
  const res = await supertest(app).delete(`/tweets/${new mongoose.Types.ObjectId()}`);
  t.is(res.status, 404);
  t.deepEqual(res.body, {
    error: 'NotFound'
  });
...
//# sourceMappingURL=/home/cocomore/microservice-sample-integration/services/microservice-sample-tweet/node_modules/.cache/ava/e43b8bf41bd3f74662f026392ebe5c50.js.map